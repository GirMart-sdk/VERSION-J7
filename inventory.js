/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
/* global genId, getSizesForCategory, stockStatus, totalStock */
...
function setInvStockFilter(filter) {
  ...
  window.setInvCategory("all"); // Esto también llamará a renderInventory
}
...
function renderInventory() {
  ...
  const catFilter =
    typeof window._invActiveCat !== "undefined" && window._invActiveCat !== "all"
      ? window._invActiveCat
      : "";
    ...
    .map((p) => {
      ...
      <div class="inv-card">
        ...
        <div class="inv-card-body">
          ...
          <div class="inv-card-price-group">
            <div class="inv-card-price">${fmt(p.price)}</div>
            <div class="inv-card-cost">Costo: ${fmt(p.cost || 0)}</div>
          </div>
  ...
  // Cálculo de valor de inventario corregido y simplificado
  const totalInventoryValue = (window.inventory || []).reduce((total, p) => {
    // La función totalStock ya suma las cantidades del objeto p.stock
    const stockCount = totalStock(p);
    return total + (Number(p.cost) || 0) * stockCount;
  }, 0);
...
window.openProductModal = (id = null) => {
  ...
  if (id) {
    ...
    if (p) {
      ...
      window.renderStockGrid(p.cat || p.category, p.stock);
    }
  } else {
    ["pName", "pPrice", "pCost", "pSku", "pImg"].forEach((f) => {
      if ($(f)) $(f).value = "";
    });
    if ($("pCat")) $("pCat").value = "";
    if ($("pCatDisplay"))
      $("pCatDisplay").textContent = "Seleccionar Categoría...";
    window.renderStockGrid("mujer", {});
  }
  window.updateStockTotal();
  $("productModal").classList.add("open");
  $("productModalOverlay").classList.add("open");
};

/**
 * Inicia el escáner de cámara desde el botón de la barra de acciones.
 */
window.startInventoryScanner = function() {
  window.startProductQRScanner(); // Reutilizamos la función de escaneo principal
}
...
window.selectCategory = (val) => {
  ...
  window.handleCategoryChange(); // Llamada directa a la función local
  window.closeCategoryPicker();
  toast(`📂 Categoría: ${val}`);
};
...
window.handleCategoryChange = function () {
  ...
  window.renderStockGrid(category, {});
  window.updateStockTotal();
};

window.renderStockGrid = function (category, stock = {}) {
  ...
  if (sizes.length === 0) {
    gridEl.innerHTML = `<div class="size-cell" style="grid-column:1/-1"><label>Cantidad (Talla Única)</label><input type="number" id="ps-U" min="0" value="${stock.U || stock.qty || 0}" oninput="window.updateStockTotal()"/></div>`;
  } else {
...
window.updateStockTotal = function () {
  ...
  const sizes = window.getSizesForCategory(cat);
  let total = 0;
...
window.showProductQR = (id) => {
  ...
  window.JsBarcode("#barcodeCanvas", barcodeValue, {
    format: "CODE128",
    lineColor: "#000",
    width: 2,
    height: 100,
        ...
        try {
          ...
          reader.onloadend = async () => {
            ...
            if (data.success) {
              ...
              window.toast("✅ Imagen recortada y subida");
              window.toast(
...
/* window.editProduct es un alias definido en la línea 269 */
/* window.deleteProduct es un alias definido en la línea 175 */
/* window.showProductQR es un alias definido en la línea 271 */
window.triggerStockUpload = triggerStockUpload;
window.handleStockUpload = handleStockUpload; // This was already defined as a global function, no need to re-assign to itself.
/* handleCategoryChange, renderInventory, etc. ya están en window */
