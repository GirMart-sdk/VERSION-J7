...
const LS = {
  ...
  set: (k, v) => {
    ...
    } catch (e) { /* Ignorar errores en modo privado */ }
  },
};
window.LS = LS;
...
const apiFetch = async (url, options = {}) => {
  ...
  return fetch(url, {
    ...
    if (res.status === 401) {
      ...
      if (typeof window.doLogout === "function") window.doLogout(true);
...
window.navigateTo = function (page) {
  ...
  // SOLUCIÓN: Forzar la actualización de datos al entrar a páginas clave
  if (page === 'payments' || page === 'sales') {
    if (typeof window.fetchSalesLog === "function") window.fetchSalesLog();
  }


  // La burbuja flotante de escaneo ha sido reemplazada por un botón en la barra de acciones de inventario.
};
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (s && s.token) {
    ...
    setTimeout(() => {
      ...
      // ¡SOLUCIÓN! Forzamos la carga de datos iniciales después del login.
      if (typeof window.fetchInventory === "function") window.fetchInventory();
      if (typeof window.fetchSalesLog === "function") window.fetchSalesLog();
  ...
  if (e.key === 'Enter') {
    if (scanBuffer.length > 2) {
      ...
      if (currentPage === "inventory" || currentPage === "pos") {
        // La lógica de escaneo físico ahora puede abrir el modal principal si se desea
        e.preventDefault();
      }
      scanBuffer = "";
    }
