...
// INYECCIÓN DINÁMICA DE IP LOCAL (Desde start-local.bat)
// Esto permite que la IP detectada por el script de inicio se agregue a la lista de confianza.
const executorIps = (process.env.ADMIN_IPS || "").split(',').filter(Boolean);
const envIps = (process.env.ALLOWED_ADMIN_IPS || "").split(',').filter(Boolean);

// Combinar y eliminar duplicados para crear la lista de confianza final
const combinedIps = [...new Set([...envIps, ...executorIps])];

if (combinedIps.length > 0) {
  process.env.ALLOWED_ADMIN_IPS = combinedIps.join(',');
  console.log(`🔌 [Server] IPs de administrador de confianza configuradas: ${process.env.ALLOWED_ADMIN_IPS}`);
}
...
app.post("/api/admin/reset-data", requireAuth, requireAdminIp, async (req, res, next) => {
  try {
    logger.warn(`🚨 [RESET TOTAL] Solicitado por ${req.user.username} desde IP: ${req.ip}`);
    // Lógica de wipe-data.js movida aquí para ser llamada vía API
    ...
    logger.info("✅ [RESET TOTAL] Base de datos transaccional limpiada con éxito.");
    res.json({ success: true, message: "Todos los datos transaccionales han sido eliminados." });
  } catch (error) {
    next(error);
  }
});
