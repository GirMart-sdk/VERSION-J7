...
(function init() {
  ...
  // --- LÓGICA DE REPORTES DE CIERRE DE CAJA ---
  // Esta función es llamada desde cash-logic.js
  window.handleSendDailyReport = async function (sessionId, downloadOnly = false) {
    if (!sessionId) {
      console.error("No se proporcionó ID de sesión para el reporte.");
      return null;
    }

    const actionText = downloadOnly ? "Generando PDF..." : "Generando y enviando reporte...";
    showToast(`⌛ ${actionText}`);

    try {
      const res = await window.apiFetch(`${window.API_URL}/arqueo/send-report`, {
        method: "POST",
      ...
      if (res.ok && data.success) {
        if (!downloadOnly) {
          showToast("✅ Reporte enviado al correo del administrador.");
        }
        // Si el backend devuelve el PDF en base64, lo convertimos a Blob
        if (data.pdfBase64) {
        ...
        throw new Error(data.error || "El servidor no pudo enviar el reporte.");
      }
    } catch (err) {
