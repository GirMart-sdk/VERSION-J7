/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
/* global ApexCharts, fetchSalesLog, getTodayStr */

let charts = {}; // Para guardar instancias de ApexCharts

/**
 * Limpia los KPIs del dashboard y muestra un mensaje de error.
 * @param {string} message - El mensaje de error a mostrar.
 */
function clearKPIsOnError(message = "Error al cargar") {
  const errorText = `<span style="color: var(--red); font-size: 12px;">${message}</span>`;
  const kpiIDs = [
    "kpiTotalRevenue", "kpiRevenueToday", "kpiOrders", "kpiOrdersToday",
    "kpiAvgTicket", "kpiTotalDebt", "kpiNetCash", "kpiConversion"
  ];
  kpiIDs.forEach(id => {
    const el = $(id);
    if (el) el.innerHTML = errorText;
  });
  if ($("stVolTotal")) $("stVolTotal").innerHTML = errorText;
  if ($("stTransTotal")) $("stTransTotal").innerHTML = errorText;
}
...
async function renderDashboard() {
  try {
    ...
    if (!statsRes.ok) {
      // Si la API falla, lanza un error para ser capturado por el bloque catch.
      const errorInfo = await statsRes.json().catch(() => ({ message: "El servidor no respondió correctamente." }));
      throw new Error(`Error ${statsRes.status}: ${errorInfo.message || "Fallo al obtener estadísticas"}`);
    }
    ...
    console.error("❌ Error al obtener analíticas principales del dashboard:", e);
    // Limpia los KPIs y muestra un mensaje de error en la UI.
    clearKPIsOnError("Error de Carga");
  }
