...
function addToPOSCart(product, size) {
  ...
  if (existing) existing.qty++;
    ...
    window.WinnerApp.pos.cart.push({
      ...
      sku: product.sku || "",
      name: product.name,
      originalPrice: product.price, // Guardamos el precio original
      price: product.price,
      size,
      qty: 1,
    });
    ...
    <div class="pos-item-row">
      <div style="flex:1">
        ...
        <div style="font-size: 11px; color: var(--gray-text);">
          Talla: ${it.size} | 
          <span class="pos-item-price-editable" onclick="editPOSItemPrice(${idx})">
            ${window.fmt(it.price)}
          </span>
          ${it.price < it.originalPrice ? `<span style="text-decoration: line-through; opacity: 0.6; margin-left: 5px;">${window.fmt(it.originalPrice)}</span>` : ""}
        </div>
      </div>
...
/**
 * Permite editar el precio de un artículo en el carrito del POS.
 * @param {number} index - El índice del artículo en el carrito.
 */
...
window.editPOSItemPrice = editPOSItemPrice;
...
window.renderPaymentsTable = function () {
  ...
  const sLog = Array.isArray(window.salesLog) ? window.salesLog : []; // Now this is the single source of truth
